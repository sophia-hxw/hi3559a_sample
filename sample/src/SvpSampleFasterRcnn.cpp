...
static const HI_CHAR *paszModelType_frcnn[] = {
...
typedef SVP_NNIE_NODE_INFO Svp_Sample_FasterRCNN_RPN_Node_Info;

static const HI_CHAR *paszRPNlayerName[] = {
...
static const HI_U32 au32ResultClassNum[] = {
    2,  // alexnet
    12, // vgg16
    3,  // resnet18
    3,  // resnet34
...
static HI_S32 SvpSampleSetRPNlayerName(Svp_Sample_FasterRCNN_RPN_Node_Info rpnNode[], HI_U8 netType)
{
    snprintf(rpnNode[RPN_NODE_INFO_ORDER_DATA].layerName, SVP_NNIE_NODE_NAME_LEN, "%s", paszRPNlayerName[netType]);
...
{
    CHECK_EXP_RET(NULL == baseAnchorInfo, HI_FAILURE, "SvpSampleAnchorInfoInit input baseAnchorInfo nullptr");
    CHECK_EXP_RET(netType < SVP_SAMPLE_WK_DETECT_NET_FASTER_RCNN_ALEX && netType >= SVP_SAMPLE_WK_DETECT_NET_FASTER_RCNN_TYPE_BUTT,
        HI_FAILURE, "SvpSampleAnchorInfoInit netType(%d) out of range[%d,%d)", netType,
...
static void getFasterRCNNParam(Faster_RCNN_Para *param, SVP_NNIE_MULTI_SEG_S *wkParam, HI_U8 netType,
    Svp_Sample_FasterRCNN_RPN_Node_Info rpnNode[])
{
    //----------RPN PAPRAMETER----------

    SVP_SAMPLE_BASE_ANCHOR_INFO_S anchorInfo;
    memset(&anchorInfo, 0, sizeof(SVP_SAMPLE_BASE_ANCHOR_INFO_S));

    (void)SvpSampleAnchorInfoInit((SVP_SAMPLE_WK_DETECT_NET_FASTER_RCNN_TYPE_E)netType, &anchorInfo);
    ...
    param->u32ClassNum = au32ResultClassNum[netType];
    ...
    if (netType != SVP_SAMPLE_WK_DETECT_NET_FASTER_RCNN_PVANET) {
        param->u32MaxRois = 300;
        param->u32NumBeforeNms = 6000;
    }
    else {
        param->u32MaxRois = 200;
    ...
    param->u32FilterThresh   = (HI_U32)(0 * SVP_WK_QUANT_BASE);        /* 20.12 fix point */
    param->u32ConfThresh     = (HI_U32)(0.3 * SVP_WK_QUANT_BASE);      /* 20.12 fix point */

}

static HI_S32 SvpSampleGetDstIndexFromLayerName(const SVP_NNIE_MODEL_S *pstModel,
...
static HI_S32 SvpSampleGetDstIndexFromSrcIndex(const SVP_NNIE_MODEL_S *pstModel,
        ...
        HI_U8 netType, HI_U32 *pu32DstAlign, HI_S32 s32Cnt)
{
    ...
    Svp_Sample_FasterRCNN_RPN_Node_Info rpnNode[FASTER_RCNN_RPN_NODE_NUM] = { 0 };
    ...
    string strNetType = paszModelType_frcnn[netType];
    ...
    if (netType != SVP_SAMPLE_WK_DETECT_NET_FASTER_RCNN_PVANET) {
        ...
        stDetCfg.u32MaxBboxNum = 200;
    }

    s32Ret = SvpSampleMultiSegCnnInit(&stDetCfg, &stDetParam, pu32DstAlign, pu32DstAlign);
    CHECK_EXP_RET(HI_SUCCESS != s32Ret, HI_FAILURE, "SvpSampleMultiSegCnnInit failed");

    // set RPN layerName by network type
    SvpSampleSetRPNlayerName(rpnNode, netType);
    ...
    for (HI_U32 i = 0; i < FASTER_RCNN_RPN_NODE_NUM; i++) {
        ...
        CHECK_EXP_RET(HI_SUCCESS != s32Ret, HI_FAILURE, "SvpSampleGetDstIndexFromLayerName failed(%s)", rpnNode[i].layerName);
    }
    ...
    // run first segment of faster_rcnn
    ...
    CHECK_EXP_GOTO(HI_SUCCESS != s32Ret, Fail, "Error(%#x): RCNN Forward failed!", s32Ret);
    ...
    // generate PRN info
    HI_MPI_SVP_NNIE_WK_CNN_FASTER_RPN_Ref1(
        conv_data,
    ...
    // run second segment of faster_rcnn
